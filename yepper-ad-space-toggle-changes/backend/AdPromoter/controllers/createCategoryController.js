...
function catToClient(c) {
  ...
  return {
    ...
    // Owner-controlled on/off switch — see setCategoryActive. Defaults true
    // for rows that predate the column (is_active is NOT NULL DEFAULT TRUE
    // going forward, but this keeps existing in-memory rows shaped from
    // before the migration ran safe too).
...
// ── setCategoryActive ────────────────────────────────────────────────────────
// Owner-facing open/close switch for one ad space. Closing it never deletes
// or unpublishes the space's config — it's the same on/off a placement stays
// wired for — it just tells the live site to stop rendering anything inside
// that space's div/box (see AdDisplayController.resolveCategoryAndAds and
// SiteScriptController's client-side loadSpace/renderAds). The div itself,
// and whatever CSS reserves its footprint, is left alone, so the layout
// doesn't jump when it's closed or reopened.
exports.setCategoryActive = async (req, res) => {
  try {
    ...
    const updated = await AdCategory.update(categoryId, { isActive });
    res.status(200).json(catToClient(updated));
  } catch (error) {
    console.error('Error setting category active state:', error);
    res.status(500).json({ message: 'Error setting category active state', error: error.message });
  }
};
