...
export async function GET(req: NextRequest, { params }: { params: any }) {
  ...
  try {
    const upstream = await fetch(`${ADSENSE_API}/api/social/youtube/ad-spaces/${encodeURIComponent(creatorId)}`, {
      method: 'GET',
    ...
    return NextResponse.json({ success: false, message: 'Network error while fetching ad spaces' }, { status: 500 });
  }
}
