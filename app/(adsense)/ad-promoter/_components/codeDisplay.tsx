...
export const MasterIntegration = ({ website, categories = [], onAddSpace, onDeleteCategory, onToggleActive, onSendInvite, onSetLanguage, onCustomize, onTargetPathChange, onDuplicated, earningsSummary, scriptInstalled = false }) => {
  ...
  const handleToggleActive = async (cat: any) => {
    ...
    setActiveTogglingId(cat._id);
    try {
      await onToggleActive(cat);
    } finally {
              ...
              <div className="border-t border-zinc-700">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4">
                  {categories.map((cat: any, idx: any) => {
                    ...
                    return (
                      <div key={cat._id} className={`rounded-2xl border bg-white overflow-hidden flex flex-col ${isCatActive ? 'border-zinc-200' : 'border-red-300 opacity-70'}`}>
                      {!isCatActive && (
                        <div className="px-4 pt-3">
                          <span className="inline-block text-[10px] font-bold uppercase tracking-wide text-red-600 bg-red-50 border border-red-200 px-2 py-0.5 rounded">
                            Closed · not showing on your site
                          </span>
                        </div>
                      )}
                      ...
                      <div className="p-4 pt-3 flex flex-wrap gap-2 mt-auto">
                        {onToggleActive && (
                          <button
                            ...
                            disabled={activeTogglingId === cat._id}
                            title={isCatActive ? 'Close this ad space: stop showing it on your site (its space on the page stays reserved)' : 'Reopen this ad space so it shows on your site again'}
                            className={`flex items-center gap-1 px-2 py-1.5 rounded text-xs font-medium transition-all border shrink-0 text-white disabled:opacity-50 ${
                              isCatActive
                                ? 'bg-zinc-800 hover:bg-red-950 hover:text-red-400 border-zinc-700 hover:border-red-900'
                                : 'bg-emerald-700 hover:bg-emerald-600 border-emerald-700'
                            }`}
                          >
                            {isCatActive ? <PowerOff className="w-3 h-3" /> : <Power className="w-3 h-3" />}
                            <span>{activeTogglingId === cat._id ? 'Updating…' : (isCatActive ? 'Close' : 'Open')}</span>
                          </button>
                        )}
                        <button
