...
async function resolveCategoryAndAds(categoryId, req) {
  ...
  // Closed by the owner (see setCategoryActive) — the space's placeholder
  // div/box stays reserved on the page, it just gets nothing rendered
  // inside it: no ad, and no "Available Advertising Space" filler either
...
exports.displayAd = async (req, res) => {
  try {
    ...
    // flag lets the site script tell this apart from "no ads yet" so it
    // doesn't show a buy-this-space card for a space the owner turned off.
    if (inactive) return res.json({ html: '', inactive: true });
...
// explicitly wants to move a div and see it reclassify immediately, not
// after several matching pageviews).
const ZONE_HISTORY_LENGTH = 1;
const ZONE_MAJORITY_THRESHOLD = 1;
