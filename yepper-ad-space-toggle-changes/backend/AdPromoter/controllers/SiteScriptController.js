...
exports.serveSiteScript = async (req, res) => {
  ...
  if (!UUID_RE.test(websiteId)) {
    ...
    const spaces = categories.map(cat => ({
      ...
      // Owner-controlled open/close switch (see setCategoryActive). false
      // means: still apply this space's positioning/sizing CSS (so the
      // page layout doesn't jump), but never fetch or render an ad or the
      // "Available Advertising Space" filler inside it.
...
(function(){
  ...
  function injectStyles(sp, data, root){
    ...
    el.textContent=fontCss+rulesCss+\`
      ...
      .\${sp.px}-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:32px 28px;text-align:center;background:#fff;box-shadow:0 8px 32px rgba(31,38,135,0.18);border-radius:16px;width:100%;height:100%;box-sizing:border-box;}
      ...
      .\${sp.px}-empty-title{font-family:Arial,Helvetica,sans-serif;font-size:20px;font-weight:400;margin:2px 0 0;max-width:260px;line-height:1.3;color:#111;}
