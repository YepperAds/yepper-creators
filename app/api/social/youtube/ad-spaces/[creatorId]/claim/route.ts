...
// Stream the multipart image upload directly to the backend without buffering it in memory.
export async function POST(req: NextRequest, { params }: { params: any }) {
  ...
  try {
    const upstream = await fetch(
      `${ADSENSE_API}/api/social/youtube/ad-spaces/${encodeURIComponent(creatorId)}/claim`,
      {
        method:  'POST',
    ...
    console.error('[proxy] ad-spaces claim error:', err);
    return NextResponse.json({ success: false, message: 'Claim proxy error' }, { status: 500 });
  }
}
