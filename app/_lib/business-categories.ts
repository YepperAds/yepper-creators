...
//
// No icons/emojis here on purpose: each category is identified purely by a
// distinct gradient + glow color (a small abstract "art" treatment), applied
// via CategoryCard.tsx — see that file for the glass/clay/neumorphic shell.
export interface BusinessCategory {
  id: string;
  label: string;
  /** CSS gradient string used as the card's background-image. */
  gradient: string;
  /** Brightest stop in the gradient — used to tint ambient shadow/glow. */
  glow: string;
}
...
// Not a real business category — a publisher-side meta-option meaning
// "accept advertisers from any category." Kept out of BUSINESS_CATEGORIES
// (which represents actual verticals shown on deals/ads) but still resolvable
// by id so CategoryCard can render it with its own distinct sweep of colors.
const ANY_CATEGORY: BusinessCategory = {
  id: 'any',
  label: 'Any category',
  gradient: 'linear-gradient(135deg, #2575fc 0%, #6a0572 33%, #e63cff 66%, #ffb96b 100%)',
  glow: '#e63cff',
};
