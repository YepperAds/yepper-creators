...
const WebsiteDetails = ({ websiteId: websiteIdProp, embedded }: { websiteId?: string; onBack?: () => void; embedded?: boolean } = {}) => {
    ...
    const handleToggleActive = async (cat) => {
        try {
            const res = await categoryAPI.setActive(cat._id, !cat.isActive);
            setCategories(categories.map(c => c._id === cat._id ? { ...c, isActive: (res.data as any).isActive } : c));
        } catch (e) {
            alert('Failed to update ad space status. Please try again.');
        }
    };
