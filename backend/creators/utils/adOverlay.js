...
// The actual ad burn-in (ffmpeg) now runs client-side in the browser — see
// app/(advertiser)/_components/lib/clientAdOverlay.ts — so this file no
// longer drives ffmpeg at all; Render never touches video bytes for that
// step. What's left here is just the ad-format catalog (corner badge vs
// L-bar, sizes) shared by the advertiser-facing claim flow, plus a small
// memory logger reused by the YouTube-upload relay job.
...
// without duplicating them. The same ratios are mirrored in
// clientAdOverlay.ts, which is what actually applies them now.
const AD_FORMATS = {
  corner: {
    label: 'Corner Badge',
