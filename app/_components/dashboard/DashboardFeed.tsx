...
// Same "click straight into the claim flow" pattern as WebsiteCard, but for
// a creator's YouTube channel — AdvertiseBrowser already reads `creatorId`
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface-2 p-4 sm:p-5 flex flex-col h-full">
      ...
      <button onClick={() => onOpen(creator)} className="relative flex items-center gap-3 w-full aspect-[4/3] shrink-0 text-left mb-4 rounded-lg overflow-hidden border border-border bg-surface-3 p-4">
        {creator.avatar ? (
          <img src={creator.avatar} alt="" className="w-14 h-14 rounded-full object-cover shrink-0" />
        ) : (
          <div className="w-14 h-14 rounded-full bg-surface-1 flex items-center justify-center text-lg font-bold text-white shrink-0">
            {(creator.channelName || creator.name).charAt(0)}
          </div>
        )}
      ...
      <button
        ...
        className="mt-auto text-xs font-semibold text-subtle underline underline-offset-2 hover:text-coral-text transition-colors text-left"
      >
        Advertise on {creator.channelName || creator.name}
      </button>
    </div>
  );
}
...
export default function DashboardFeed({ websites, creators = [] }: { websites: PublicWebsite[]; creators?: PublicCreator[] }) {
  ...
  const openCreator = (creator: PublicCreator) => router.push(`/?panel=advertise&creatorId=${creator.id}`, { scroll: false });

  if (websites.length === 0 && creators.length === 0) {
    return (
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {creators.map((creator) => (
        <CreatorCard key={`creator-${creator.id}`} creator={creator} onOpen={openCreator} />
      ))}
