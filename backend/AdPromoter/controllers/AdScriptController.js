  ...
  function injectStyles(data){
    ...
    el.textContent=placementCss+fontCss+rulesCss+hostOverride+\`
      ...
      /* Short, banner-shaped spaces get a single-row filler instead of the
         ...
         SiteScriptController.js's injectStyles. Same rounded corners as the
         full card so the tier box-shadow ring lines up with the card's own
         edge instead of poking past it at the corners. */
      .\${_px}-empty-compact{flex-direction:row;justify-content:space-between;align-items:center;gap:16px;padding:0 24px;text-align:left;border-radius:16px;box-shadow:none;background:#fff;}
      .\${_px}-empty-compact .\${_px}-empty-price{font-family:Arial,Helvetica,sans-serif;font-size:22px;font-weight:400;color:#111;margin:0;line-height:1.3;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
      ...
      /* Multi-tier ad spaces: pricier of the 3 slots visibly looks pricier —
         ...
         the matching rules on the primary site-wide script. Box-shadow ring,
         radius pinned explicitly so it always matches the card's own rounded
         corners instead of showing square. Card background stays plain
         white for every tier — only the button and this ring carry color. */
      .\${_px}-ad[data-tier-rank="1"]{border:none!important;border-radius:16px!important;box-shadow:0 0 0 2px #FFD500,0 10px 26px rgba(0,0,0,0.35)!important;}
      .\${_px}-ad[data-tier-rank="2"]{border:none!important;border-radius:16px!important;box-shadow:0 0 0 2px #E8472B,0 10px 26px rgba(255,138,128,0.45)!important;}
      ...
      .\${_px}-badge{position:absolute;top:8px;left:8px;z-index:3;display:inline-flex;align-items:center;padding:3px 10px;border-radius:20px;background:#FFD500;color:#000;font-size:10px;font-weight:800;letter-spacing:0.04em;text-transform:uppercase;box-shadow:0 2px 6px rgba(0,0,0,0.25);}
      .\${_px}-badge-top{background:#E8472B;color:#fff;}

      /* Same escalation applied to an unsold slot's own pitch — see the
         matching note in SiteScriptController.js's injectStyles. Each tier
         gets its own distinct font on "Advertise Here". Rank 1's button
         carries a thin black inset ring (yellow with a touch of black);
         rank 2's fades from red into a soft pink at one edge (red with a
         little pink in it). */
      .\${_px}-ad[data-tier-rank="1"] .\${_px}-empty-title,
      .\${_px}-ad[data-tier-rank="1"] .\${_px}-empty-compact .\${_px}-empty-price{font-family:Georgia,'Times New Roman',serif;font-size:23px;font-weight:400;}
      .\${_px}-ad[data-tier-rank="1"] .\${_px}-empty-cta{background:#FFD500;color:#000;box-shadow:0 0 0 1.5px rgba(0,0,0,0.55) inset;}
      .\${_px}-ad[data-tier-rank="2"] .\${_px}-empty-title,
      .\${_px}-ad[data-tier-rank="2"] .\${_px}-empty-compact .\${_px}-empty-price{font-family:'Courier New',Courier,monospace;font-size:26px;font-weight:400;}
    ...
    if(_sp&&!ZONE_EXEMPT[_sp.toLowerCase()]){
      setTimeout(function(){
        try{
          ...
          var topPct=absTop/pageH, bottomPct=absBottom/pageH;
          /* Same band split as SiteScriptController.js's detectZone — see
...
exports.serveAdEmbed = async (req, res) => {
  try {
    ...
    const { adCategory, ads, inactive } = await resolveCategoryAndAds(categoryId, req);
    if (!adCategory) return res.status(404).send(blankEmbedPage());
    // Closed by the owner (see setCategoryActive) — the iframe stays where
    // it's embedded, it just renders blank instead of an ad or the "buy
    // this space" filler.
    if (inactive) return res.status(200).send(blankEmbedPage());
