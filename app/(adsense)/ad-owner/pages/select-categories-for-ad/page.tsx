...
const SelectCategoriesForExistingAd = () => {
  ...
  const [categoriesByWebsite, setCategoriesByWebsite] = useState<Record<string,unknown>[]>([]);
  const [selectedCategories, setSelectedCategories] = useState<Record<string,unknown>[]>([]);
  const [loading, setIsLoading] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showPaymentSummary, setShowPaymentSummary] = useState(false);
  const [paymentSelections, setPaymentSelections] = useState<Record<string,unknown>[]>([]);
  const [setWalletInfo] = useState({ balance: 0, hasWallet: false });
  const [expandedCategory, setExpandedCategory] = useState<unknown>(null);
  ...
  const calculatePaymentBreakdown = async () => {
    try {
      ...
      if ((error as any).response?.data?.code === 'REFUND_NOT_ALLOWED_FOR_REASSIGNMENT') {
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      if ((error as any).response?.data?.code === 'REFUND_NOT_ALLOWED_FOR_REASSIGNMENT') {
        ...
        setError('Error adding website selections: ' + ((error as any).response?.data?.error || 'Unknown error'));
      }
    } finally {
      setIsSubmitting(false);
    }
