...
async function startServer() {
  ...
  // Owner-controlled open/close switch for an ad space (is_active).
  try {
    const { up: addIsActive } = require('./migrations/20260903_ad_category_is_active');
    await addIsActive();
    console.log('✓ ad space open/close switch ready');
  } catch (err) {
    console.warn('⚠️  Migration 20260903_ad_category_is_active skipped:', err.message?.split('\n')[0]);
  }
